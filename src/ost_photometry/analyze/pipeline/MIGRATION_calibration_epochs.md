...
Differential-calibration APIs use **epoch** naming (`add_epoch`, `epochs`, `epoch_metadata`, `calibrate_epoch`, `fit_extinction_from_epochs`, table column `epoch_id`). The bridge still exposes `observation_to_frame_tables` as an alias of `observation_to_epoch_tables`.
...
`observation_to_epoch_tables` (and alias `observation_to_frame_tables`) returns `dict(context.calibration_epochs)` (empty if epochs were never built).
