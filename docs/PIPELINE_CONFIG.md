...
- **Known variables** — before the fit, catalog stars that coincide with [VSX](https://cdsarc.cds.unistra.fr/viz-bin/cat/B/vsx) (default `1 arcsec`) are dropped so they never get `mag_std_*`. Same behaviour as the old calibration path. Disable with `exclude_known_variables=False`. A failed VSX query leaves the catalog unchanged and does not abort calibration.
