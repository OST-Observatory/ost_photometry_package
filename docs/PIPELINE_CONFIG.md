...
**Frame WCS** — matching uses `image.wcs` per frame. `image_series.wcs` is the reference-image WCS (plots, FOV, catalog cone). Calibration epoch RA/Dec also use `image.wcs` (not the series handle), so catalog cross-match follows the same sky positions as tracking. `wcs_solve_all_images=True` solves every frame and does **not** broadcast that solution. After a successful `aa_true` / `wcs` warp, copying the reference WCS onto aligned frames is the current solution for that grid.

**`correlation_link_mode`** — `"to_reference"` (default) matches every frame to the reference on the sky. Use this when each exposure has its own WCS (C7 with `wcs_solve_all_images=True`): pointing drift is in the WCS, and identity stays the reference star. `"sequential"` seeds tracks on the reference frame, chain-matches neighbours in both time directions (so stars that appear off-reference still get a track), then **drops** members that have walked farther than `separation_limit` from that sky anchor. Sequential without that anchor can assign a neighbour to the same `id` after a night gap — check-star LCs then show discrete magnitude jumps.
