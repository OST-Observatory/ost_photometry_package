...
**`correlation_link_mode`** — `"to_reference"` (default) matches every frame to the reference. `"sequential"` seeds on the reference and chain-matches neighbours, then drops members that walk off the anchor. Sequential vs to_reference barely matters if matching is still on noisy per-frame WCS; pixel matching on an aligned grid is the identity-preserving mode for C7.
