...
| `magnitude_limit_image_annotation` | float or `None` (default `None`)                           | Simbad annotate                 | TAP + plot: keep objects brighter than this mag. Band from `filter_magnitude_limit_image_annotation` (default `V` when the limit is set). |
