...
| `PipelineConfig.calibration_catalog_radius_arcmin` | (unchanged; was briefly `differential_apass_radius`) |
| Single faint limit (old differential default ~16 mag) | Upper bound of `calibration_catalog_mag_range`, e.g. `(0.0, 16.0)` |
...
Epoch-native calibration previously used a single faint limit (`mag_limit=16`);
