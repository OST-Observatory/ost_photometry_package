...
<output>/
  ...
  work/
    ...
    subtract/        # HiPS cache + difference FITS (optional step)
```
...
| `lightcurve_nights_jd_<name>_<filter>` | Same source, colour/marker per local night (C7 `4_compare_nights.py`). |
...
calibration). Compare nights with `4_compare_nights.py` (one ECSV that
