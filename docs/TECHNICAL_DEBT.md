...
| `reduce/registration.py` further package split | P3 | Trim helpers already unified (`trim_ccd`); remaining in-code TODOs inventoried in [TODO.md](TODO.md) |
| In-code `TODO` markers in `src/` | — | **11** verified (`rg 'TODO' src`); concrete backlog under [TODO.md § In-code TODOs](TODO.md#in-code-todos) |
...
| `old_legacy_function/` | Archive only; not part of the package API. Still imports removed APIs (`calibration_data`, `derive_calibration`, …) and is not runnable. |
...
Removed `calibration_data` / `derive_calibration` / `legacy_adapter` / `CalibParameters` (protect path uses `calibration_sources`); magnitude filter-set / Vega–AB model; T/ZP `cov_tz`; `maximum_n_eps_stars`; ASCII `formats` `i` vs `id`; calibration convergence, extinction modes, mk_calib engine path, reduce workflow modularization, utils/plots/calibration splits, `Image`/`AnalysisImage`, FWHM helpers, registration trim unification, protect-calibrators, diagnostic plot hooks — details in git history and [ARCHITECTURE_AND_MIGRATION.md](ARCHITECTURE_AND_MIGRATION.md).
