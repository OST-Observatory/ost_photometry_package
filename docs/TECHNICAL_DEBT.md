...
| ~~`utilities.py` — split `Image`~~ | P3 | **Done** | Base `ost_photometry.image.Image` (metadata/I/O); analysis state on `analyze.image.AnalysisImage`. Re-exported as `utilities.Image`. |
...
| `Image` / `AnalysisImage` split | Done — `ost_photometry/image.py` + `analyze/image.py`; `test_image_split.py` |
